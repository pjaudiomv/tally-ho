...
<table id="meetingVenueReport" class="tallyTable">
	...
	<tr>
		<td>virtual</td>
		<td>{tally.meetings.reduce((sum, meeting) => sum + meeting.num_virtual, 0)}</td>
	</tr>
	<tr>
		<td>in_person</td>
		<td>{tally.meetings.reduce((sum, meeting) => sum + meeting.num_in_person, 0)}</td>
	</tr>
	<tr>
		<td>hybrid</td>
		<td>{tally.meetings.reduce((sum, meeting) => sum + meeting.num_hybrid, 0)}</td>
	</tr>
	</tbody>
</table>
